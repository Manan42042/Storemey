...
app.controller('storeStateMastersCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeStateMaster', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreStateMasters, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeStateMasters = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreStateMaster', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreStateMaster', { SearchDto: $scope.SearchDto });
        };


        var DeletestoreStateMasterInputDto = {
        };
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreStateMasterInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'State, name: <strong>' + entity.stateName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreStateMasters.delete(DeletestoreStateMasterInputDto).then(function (result) {
        ...
        //    _thisServicesstoreStateMasters.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeStateMaster/importModal.cshtml',
                    controller: 'importstoreStateMasterCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreStateMasters = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "stateName", displayName: "State name", width: '45%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "countryName", displayName: "Country name", width: '45%', headerCellClass: $scope.highlightFilteredHeader },
                    {
                        name: 'Commands',
                        enableFiltering: false,
                        enableSorting: false,
            ...
            ////var apiRoute = 'api/storeStateMaster/GetstoreStateMasters/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreStateMasters(apiRoute);
            ...
            //    $('.grid'),
                ...
                _thisServicesstoreStateMasters.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

...
app.controller('storeProductssCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeProducts','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreProductss, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeProductss = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreProducts', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreProducts', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreProductsInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreProductsInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Product, name: <strong>' + entity.ProductName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreProductss.delete(DeletestoreProductsInputDto).then(function (result) {
        ...
        //    _thisServicesstoreProductss.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeProducts/importModal.cshtml',
                    controller: 'importstoreProductsCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreProductss = function () {
                ...
                columnDefs: [
                    //{ name: "id", displayName: "Number", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    {
                        ...
                        cellTemplate: '<span class="label-mini">' +
                            '<img src="/Image/GetImage?id={{row.entity.defaultImage}}&foldername=productImage&Size=50&{{::$id}}" style="border: 3px solid #eee;border-radius: 5px;margin: 2px;" />' +
                            '</span>'
                    },
                    { name: "productName", displayName: "Product Name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "supplier", displayName: "Supplier", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "variants", displayName: "Variants", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    //{ name: "inventory", displayName: "Inventory", width: '12%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "date", displayName: "Date", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeProducts/GetstoreProductss/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreProductss(apiRoute);
            ...
            cfpLoadingBar.start();
                ...
                _thisServicesstoreProductss.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

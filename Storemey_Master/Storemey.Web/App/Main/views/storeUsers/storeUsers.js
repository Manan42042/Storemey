...
app.controller('storeUsersCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeUsers','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreUserss, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeUserss = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreUsers', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreUsers', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreUsersInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreUsersInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'User, name: <strong>' + entity.firstName + ' ' + entity.lastName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreUserss.delete(DeletestoreUsersInputDto).then(function (result) {
        ...
        //    _thisServicesstoreUserss.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeUsers/importModal.cshtml',
                    controller: 'importstoreUsersCTR',
                    backdrop: 'static'
                });
        ...
        $scope.GetstoreUserss = function () {
                ...
                columnDefs: [
...

                    {
                        ...
                        cellTemplate: '<span class="label-mini">' +
                            '<img src="/Image/GetImage?id={{row.entity.id}}&foldername=userImages&Size=50&{{::$id}}" style="border: 3px solid #eee;border-radius: 5px;margin: 2px;" />' +
                            '</span>'
                    },

                    { name: "firstName", displayName: "Firstname", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "lastName", displayName: "Lastname", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "userName", displayName: "Username", width: '12%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "emailAddress", displayName: "Email", width: '12%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "phoneNumber", displayName: "Phone number", width: '12%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "role", displayName: "Role", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "outlet", displayName: "Outlets", width: '12%', headerCellClass: $scope.highlightFilteredHeader },
                    //{ name: "lastModificationTime", displayName: "Updated Date", type: 'date', width: '10%', cellFilter: 'date:\'MM-dd-yyyy hh:mm a\'', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeUsers/GetstoreUserss/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreUserss(apiRoute);
            ...
            //    $('.grid'),
            cfpLoadingBar.start();
                _thisServicesstoreUserss.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

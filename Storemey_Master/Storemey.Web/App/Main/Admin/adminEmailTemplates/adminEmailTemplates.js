...
angular.module('app').controller('adminEmailTemplatesCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.adminEmailTemplates', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesadminEmailTemplates, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.adminEmailTemplates = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditadminEmailTemplates', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditadminEmailTemplates', { SearchDto: $scope.SearchDto });
        }

        var DeleteadminEmailTemplatesInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeleteadminEmailTemplatesInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesadminEmailTemplates.delete(DeleteadminEmailTemplatesInputDto).then(function (result) {
        ...
        //    _thisServicesadminEmailTemplates.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/adminEmailTemplates/importModal.cshtml',
                    controller: 'importadminEmailTemplatesCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetadminEmailTemplates = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "fromEmail", title: "From Email", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "ccEmail", title: "CC Email", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "bccEmail", title: "BCC Email", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "emailKey", title: "EmailKey", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "subject", title: "Subject", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    {
                        name: 'Commands',
                        enableFiltering: false,
                        enableSorting: false,
            ...
            //var apiRoute = 'api/adminEmailTemplates/GetadminEmailTemplates/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getadminEmailTemplates(apiRoute);
            ...
            _thisServicesadminEmailTemplates.getAdvanceSearch($scope.SearchDto).then(function (result) {
                if (result.data === null) {

...
angular.module('app').controller('adminStoresCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.adminStores','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesadminStores, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.adminStores = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditadminStores', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditadminStores', { SearchDto: $scope.SearchDto });
        }

        var DeleteadminStoresInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeleteadminStoresInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesadminStores.delete(DeleteadminStoresInputDto).then(function (result) {
        ...
        //    _thisServicesadminStores.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/adminStores/importModal.cshtml',
                    controller: 'importadminStoresCTR',
                    backdrop: 'static'
                });
        ...
        $scope.GetadminStores = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "storeName", title: "Store Name", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "userName", title: "UserName", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "email", title: "Email", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "planID", title: "PlanID", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "expirationDate", title: "Expiration Date", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
...

                    {
                        ...
                        cellTemplate: '<span class="label-mini">' +
                            ...
                            ' <a href="#" class="btn btn-transparent btn-xs tooltips" tooltip-placement="top"  ng-click="grid.appScope.remove(row.entity)" uib-tooltip="Remove" style="padding: 0px; margin-left: 3px;" >' +
            ...
            //var apiRoute = 'api/adminStores/GetadminStores/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getadminStores(apiRoute);
            ...
            //    $('.grid'),
                ...
                _thisServicesadminStores.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

...
namespace LowercaseDashedRouting
{
	public class DashedRouteHandler : MvcRouteHandler
	{
		...
		protected override IHttpHandler GetHttpHandler(RequestContext requestContext)
		{
			...
			values["action"] = string.IsNullOrEmpty(Convert.ToString(values["action"])) ? "SiteMaster" : UnDash(values["action"].ToString());//UnDash(values["action"].ToString());
            values["controller"] = string.IsNullOrEmpty(Convert.ToString(values["controller"])) ? "home" : UnDash(values["controller"].ToString());

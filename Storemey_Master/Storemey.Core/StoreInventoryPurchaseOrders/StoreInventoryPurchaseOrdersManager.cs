...
using Storemey.StoreInventoryPurchaseOrders;
using Storemey.Authorization.Users;
using System.Data.Entity.SqlServer;

namespace Storemey.StoreInventoryPurchaseOrders
{
    public class StoreInventoryPurchaseOrdersManager : IStoreInventoryPurchaseOrdersManager
    {
        private readonly IRepository<StoreInventoryPurchaseOrders, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreInventoryPurchaseOrdersManager(
            IRepository<StoreInventoryPurchaseOrders, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreInventoryPurchaseOrders>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreInventoryPurchaseOrders>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)


        //public async Task<IQueryable<StoreInventoryPurchaseOrders>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
        ...
        public async Task Create(StoreInventoryPurchaseOrders input)
        {
            try
            {
        ...
        public async Task Update(StoreInventoryPurchaseOrders input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreInventoryPurchaseOrders input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreInventoryPurchaseOrders> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreInventoryPurchaseOrders();
            }
            return @MasterCountries;
        }

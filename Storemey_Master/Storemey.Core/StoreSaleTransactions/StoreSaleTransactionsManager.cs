...
using Storemey.StoreSaleTransactions;
using Storemey.Authorization.Users;
using System.Data.Entity.SqlServer;

namespace Storemey.StoreSaleTransactions
{
    public class StoreSaleTransactionsManager : IStoreSaleTransactionsManager
    {
        private readonly IRepository<StoreSaleTransactions, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreSaleTransactionsManager(
            IRepository<StoreSaleTransactions, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreSaleTransactions>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreSaleTransactions>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
        ...
        public async Task Create(StoreSaleTransactions input)
        {
            try
            {
        ...
        public async Task Update(StoreSaleTransactions input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreSaleTransactions input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreSaleTransactions> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreSaleTransactions();
            }
            return @MasterCountries;
        }

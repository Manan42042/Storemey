...
{
    public class StoreProductBrandLinksManager : IStoreProductBrandLinksManager
    {
        private readonly IRepository<StoreProductBrandLinks, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreProductBrandLinksManager(
            IRepository<StoreProductBrandLinks, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreProductBrandLinks>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreProductBrandLinks>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreProductBrandLinks>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                             || x.BrandId.ToString().Contains(SearchText)
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "brandName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.BrandId);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "brandName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.BrandId);
                                break;
        ...
        public async Task Create(StoreProductBrandLinks input)
        {
            try
            {
        ...
        public async Task Update(StoreProductBrandLinks input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreProductBrandLinks input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreProductBrandLinks> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreProductBrandLinks();
            }
            return @MasterCountries;
        }
        ...
        public async Task<List<StoreProductBrandLinks>> GetByProductId(long ID)
        {
            var @MasterCountries = await _thisRpository.GetAllListAsync(x => x.IsDeleted == false && x.ProductId == ID);

            if (@MasterCountries == null)
            {
                return new List<StoreProductBrandLinks>();
            }
            return @MasterCountries;
        }

...
{
    public class StoreProductSeasonLinksManager : IStoreProductSeasonLinksManager
    {
        private readonly IRepository<StoreProductSeasonLinks, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreProductSeasonLinksManager(
            IRepository<StoreProductSeasonLinks, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreProductSeasonLinks>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreProductSeasonLinks>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                             || x.SeasonId.ToString().Contains(SearchText)
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "seasonId":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.SeasonId);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "seasonIdeasonId":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.SeasonId);
                                break;
        ...
        public async Task Create(StoreProductSeasonLinks input)
        {
            try
            {
        ...
        public async Task Update(StoreProductSeasonLinks input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreProductSeasonLinks input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreProductSeasonLinks> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreProductSeasonLinks();
            }
            return @MasterCountries;
        }
        ...
        public async Task<List<StoreProductSeasonLinks>> GetByProductId(long ID)
        {
            var @MasterCountries = await _thisRpository.GetAllListAsync(x => x.IsDeleted == false && x.ProductId == ID);

            if (@MasterCountries == null)
            {
                return new List<StoreProductSeasonLinks>();
            }
            return @MasterCountries;
        }

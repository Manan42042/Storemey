...
{
    public class StoreBrandsManager : IStoreBrandsManager
    {
        private readonly IRepository<StoreBrands, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreBrandsManager(
            IRepository<StoreBrands, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreBrands>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreBrands>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreBrands>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                             || x.BrandName.Contains(SearchText)
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "brandName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.BrandName);
                                break;
                            case "description":
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "brandName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.BrandName);
                                break;
                            case "description":
        ...
        public async Task Create(StoreBrands input)
        {
            try
            {

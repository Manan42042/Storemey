...
using Storemey.StoreCustomers;
using Storemey.Authorization.Users;
using System.Data.Entity.SqlServer;

namespace Storemey.StoreCustomers
{
    public class StoreCustomersManager : IStoreCustomersManager
    {
        private readonly IRepository<StoreCustomers, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreCustomersManager(
            IRepository<StoreCustomers, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreCustomers>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreCustomers>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreCustomers>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "firstName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.FirstName);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "firstName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.FirstName);
                                break;
        ...
        public async Task Create(StoreCustomers input)
        {
            try
            {

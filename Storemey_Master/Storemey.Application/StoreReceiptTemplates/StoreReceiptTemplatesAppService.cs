...
namespace Storemey.StoreReceiptTemplates
{
    ...
    public class StoreReceiptTemplatesAppService : AbpServiceBase, IStoreReceiptTemplatesAppService
    {
        ...
        public async Task Create(CreateStoreReceiptTemplatesInputDto input)
        {
            ...
            if (!string.IsNullOrEmpty(input.Logo) && input.Logo.Contains("base64"))
            {
                CommonEntityHelper.Savenewimage(input.Logo,HostingEnvironment.ApplicationPhysicalPath + backslace + folderROOT + backslace + StoremeyConsts.StoreName + backslace + folderImage + backslace + storeImage + backslace, "rt_" + mapData.Id.ToString(), "");
            }
        ...
        public async Task Update(UpdateStoreReceiptTemplatesInputDto input)
        {
            ...
            if (!string.IsNullOrEmpty(input.Logo) && input.Logo.Contains("base64"))
            {
                CommonEntityHelper.Savenewimage(input.Logo,HostingEnvironment.ApplicationPhysicalPath + backslace + folderROOT + backslace + StoremeyConsts.StoreName + backslace + folderImage + backslace + storeImage + backslace, "rt_" + mapData.Id.ToString(), mapData.Id.ToString());
            }
        ...
        public async Task<ListResultDto<GetStoreReceiptTemplatesOutputDto>> GetAdvanceSearch(StoreReceiptTemplatesAdvanceSearchInputDto input)
        {
            ...
            var mapDataquery = filtereddatatquery.Item1.MapTo<List<GetStoreReceiptTemplatesOutputDto>>();

...
{
    public class StoreCountryMasterManager : IStoreCountryMasterManager
    {
        private readonly IRepository<StoreCountryMaster, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreCountryMasterManager(
            IRepository<StoreCountryMaster, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreCountryMaster>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreCountryMaster>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreCountryMaster>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection = "desc")
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                             || (x.CountryName.Contains(SearchText) || x.CountryCode.Contains(SearchText))
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "countryName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.CountryName);
                                break;
                            case "countryCode":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.CountryCode);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "countryName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.CountryName);
                                break;
                            case "countryCode":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.CountryCode);
                                break;
                            case "lastModificationTime":
        ...
        public async Task Create(StoreCountryMaster input)
        {
            try
            {

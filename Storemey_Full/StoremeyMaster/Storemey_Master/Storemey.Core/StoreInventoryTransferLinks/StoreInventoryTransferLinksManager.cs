...
using Storemey.StoreInventoryTransferLinks;
using Storemey.Authorization.Users;
using System.Data.Entity.SqlServer;

namespace Storemey.StoreInventoryTransferLinks
{
    public class StoreInventoryTransferLinksManager : IStoreInventoryTransferLinksManager
    {
        private readonly IRepository<StoreInventoryTransferLinks, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreInventoryTransferLinksManager(
            IRepository<StoreInventoryTransferLinks, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreInventoryTransferLinks>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreInventoryTransferLinks>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreInventoryTransferLinks>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
        ...
        public async Task Create(StoreInventoryTransferLinks input)
        {
            try
            {
        ...
        public async Task Update(StoreInventoryTransferLinks input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreInventoryTransferLinks input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreInventoryTransferLinks> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreInventoryTransferLinks();
            }
            return @MasterCountries;
        }

...
namespace Storemey.StoreProductImages
{
    public class StoreProductImagesManager : IStoreProductImagesManager
    {
        ...
        public async Task<List<StoreProductImages>> ListAllByProduct(long ProductId)
        {
            try
            {
                var @MasterCountries = await _thisRpository.GetAllListAsync(x => x.IsDeleted == false && x.ProductId == ProductId);
                if (@MasterCountries == null || @MasterCountries.Count == 0)
                {
                    StoreProductImages o1 = new StoreProductImages();
                    List<StoreProductImages> l1 = new List<StoreProductImages>();
                    l1.Add(o1);
                    return l1;
                }

...
{
    public class StoreProductVariantsManager : IStoreProductVariantsManager
    {
        private readonly IRepository<StoreProductVariants, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreProductVariantsManager(
            IRepository<StoreProductVariants, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreProductVariants>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreProductVariants>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreProductVariants>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                           || (x.VariantName.Contains(SearchText))
                ...
                switch (SortDirection)
                {
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            ...
                            case "variantName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.VariantName);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            ...
                            case "variantName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.VariantName);
                                break;
        ...
        public async Task Create(StoreProductVariants input)
        {
            try
            {
        ...
        public async Task Update(StoreProductVariants input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreProductVariants input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreProductVariants> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreProductVariants();
            }
            return @MasterCountries;
        }

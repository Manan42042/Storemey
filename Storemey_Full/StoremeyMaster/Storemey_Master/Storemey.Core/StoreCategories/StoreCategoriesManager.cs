...
{
    public class StoreCategoriesManager : IStoreCategoriesManager
    {
        private readonly IRepository<StoreCategories, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreCategoriesManager(
            IRepository<StoreCategories, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreCategories>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreCategories>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)


        //public async Task<IQueryable<StoreCategories>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "categoryName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.CategoryName);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "categoryName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.CategoryName);
                                break;
        ...
        public async Task Create(StoreCategories input)
        {
            try
            {
        ...
        public async Task Update(StoreCategories input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreCategories input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreCategories> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreCategories();
            }
            return @MasterCountries;
        }


        public async Task<long> GetMaxID(long ID)
        {
            var @MasterCountries = _thisRpository.GetAllList().Max(x=>x.Id);

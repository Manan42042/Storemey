...
using Storemey.StoreInventoryTransferOrders;
using Storemey.Authorization.Users;
using System.Data.Entity.SqlServer;

namespace Storemey.StoreInventoryTransferOrders
{
    public class StoreInventoryTransferOrdersManager : IStoreInventoryTransferOrdersManager
    {
        private readonly IRepository<StoreInventoryTransferOrders, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreInventoryTransferOrdersManager(
            IRepository<StoreInventoryTransferOrders, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreInventoryTransferOrders>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreInventoryTransferOrders>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreInventoryTransferOrders>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
        ...
        public async Task Create(StoreInventoryTransferOrders input)
        {
            try
            {
        ...
        public async Task Update(StoreInventoryTransferOrders input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreInventoryTransferOrders input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreInventoryTransferOrders> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreInventoryTransferOrders();
            }
            return @MasterCountries;
        }

...
{
    public class StoreProductVariantsLinksManager : IStoreProductVariantsLinksManager
    {
        private readonly IRepository<StoreProductVariantsLinks, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreProductVariantsLinksManager(
            IRepository<StoreProductVariantsLinks, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreProductVariantsLinks>> ListAll()
        {
            try
            {
        ...
        public async Task<List<StoreProductVariantsLinks>> ListAllProductAndOutlet(long ProductId, long OutletId)
        {
            try
            {
                var @MasterCountries = _thisRpository.GetAllList(x => x.IsDeleted == false && x.ProductId == ProductId && x.VariantId == OutletId);
                if (@MasterCountries == null || @MasterCountries.Count == 0)
                {
                    StoreProductVariantsLinks o1 = new StoreProductVariantsLinks();
                    List<StoreProductVariantsLinks> l1 = new List<StoreProductVariantsLinks>();
                    l1.Add(o1);
                    @MasterCountries = l1;
                }
        ...
        public async Task<Tuple<IQueryable<StoreProductVariantsLinks>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)

        //public async Task<IQueryable<StoreProductVariantsLinks>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
        ...
        public async Task Create(StoreProductVariantsLinks input)
        {
            try
            {
        ...
        public async Task Update(StoreProductVariantsLinks input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreProductVariantsLinks input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreProductVariantsLinks> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreProductVariantsLinks();
            }
            return @MasterCountries;
        }
        ...
        public async Task<List<StoreProductVariantsLinks>> GetByProductId(long ID)
        {
            var @MasterCountries = await _thisRpository.GetAllListAsync(x => x.IsDeleted == false && x.ProductId == ID);

            if (@MasterCountries == null)
            {
                return new List<StoreProductVariantsLinks>();
            }
            return @MasterCountries;
        }

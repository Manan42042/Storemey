...
app.controller('storeCategoriessCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeCategories','cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreCategoriess, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeCategoriess = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreCategories', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreCategories', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreCategoriesInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreCategoriesInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Category, name: <strong>' + entity.categoryName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreCategoriess.delete(DeletestoreCategoriesInputDto).then(function (result) {
        ...
        //    _thisServicesstoreCategoriess.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeCategories/importModal.cshtml',
                    controller: 'importstoreCategoriesCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreCategoriess = function () {
                ...
                columnDefs: [
...

                    {
                        ...
                        cellTemplate: '<span class="label-mini">' +
                            '<img src="/Image/GetImage?id={{row.entity.id}}&foldername=CatergoryImage&Size=50&{{::$id}}" style="border: 3px solid #eee;border-radius: 5px;margin: 2px;" />' +
                            '</span>'
                    },

                    { name: "categoryName", displayName: "Category Name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "description", displayName: "Description", width: '50%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeCategories/GetstoreCategoriess/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreCategoriess(apiRoute);
            ...
            _thisServicesstoreCategoriess.getAdvanceSearch($scope.SearchDto).then(function (result) {
                if (result.data === null) {

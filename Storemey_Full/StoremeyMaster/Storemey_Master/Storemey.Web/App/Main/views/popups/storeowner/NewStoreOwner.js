(function () {
    ...
    angular.module('app').controller('app.views.storeOwner', [
        '$scope', '$state', '$stateParams', 'abp.services.app.adminStores', 'abp.services.app.masterService','$rootScope',
        function ($scope, $state, $stateParams, _thisServices, _masterService, $rootScope) {
            ...
            $scope.saveAddEdit = function () {
                ...
                $scope.GetadminStoresrOutputDto.currancyId = $scope.GetadminStoresrOutputDto.cur.id;
                $scope.GetadminStoresrOutputDto.timeZone = $scope.GetadminStoresrOutputDto.time.name;
                $scope.GetadminStoresrOutputDto.timeZoneId = $scope.GetadminStoresrOutputDto.time.id;

                localStorage.setItem('outletName', $scope.GetadminStoresrOutputDto.out.outletName);
                localStorage.setItem('registerName', $scope.GetadminStoresrOutputDto.reg.registerName);

                $rootScope.localStorageoutletName = $scope.GetadminStoresrOutputDto.out.outletName;
                $rootScope.localStorageregisterName = $scope.GetadminStoresrOutputDto.reg.registerName;
                ...
                if ($scope.GetadminStoresrOutputDto.id === undefined || $scope.GetadminStoresrOutputDto.id === 0) {
                    ...
                    _thisServices.update($scope.GetadminStoresrOutputDto)
                        .then(function () {
                            ...
                            _thisServices.updateUsersTables($scope.GetadminStoresrOutputDto)
                                .then(function () {
                                    abp.notify.success("Saved Successfully.");
                                    $scope.modalInstance.close();
                                    $scope.modalInstance.dismiss('cancel');

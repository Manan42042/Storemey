...
app.controller('storeTaxGroupssCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeTaxGroups','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreTaxGroupss, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeTaxGroupss = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreTaxGroups', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreTaxGroups', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreTaxGroupsInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreTaxGroupsInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Tax group, name: <strong>' + entity.taxGroupName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreTaxGroupss.delete(DeletestoreTaxGroupsInputDto).then(function (result) {
        ...
        //    _thisServicesstoreTaxGroupss.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeTaxGroups/importModal.cshtml',
                    controller: 'importstoreTaxGroupsCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreTaxGroupss = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "taxGroupName", displayName: "Group name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "taxIncluded", displayName: "Tax included", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "grouprate", displayName: "Group rate (%)", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeTaxGroups/GetstoreTaxGroupss/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreTaxGroupss(apiRoute);
            ...
            //    $('.grid'),
            cfpLoadingBar.start();
                _thisServicesstoreTaxGroupss.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

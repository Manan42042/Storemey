...
angular.module('app').controller('adminPlanPriceCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.masterCountries', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesadminPlanPrice) {
        $scope.gridOptions = [];
        $scope.adminPlanPrice = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditadminPlanPrice', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditadminPlanPrice', { SearchDto: $scope.SearchDto });
        }

        var DeleteadminPlanPriceInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeleteadminPlanPriceInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesadminPlanPrice.delete(DeleteadminPlanPriceInputDto).then(function (result) {
        ...
        //    _thisServicesadminPlanPrice.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/adminPlanPrice/importModal.cshtml',
                    controller: 'importadminPlanPriceCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetadminPlanPrice = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "planID", title: "Plan ID", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "countryID", title: "Country ID", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "amount", title: "Amount", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/adminPlanPrice/GetadminPlanPrice/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getadminPlanPrice(apiRoute);
            ...
            cfpLoadingBar.start();
                ...
                _thisServicesadminPlanPrice.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

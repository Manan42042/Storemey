...
app.controller('storeTagssCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeTags', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreTagss, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeTagss = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreTags', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreTags', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreTagsInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreTagsInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreTagss.delete(DeletestoreTagsInputDto).then(function (result) {
        ...
        //    _thisServicesstoreTagss.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeTags/importModal.cshtml',
                    controller: 'importstoreTagsCTR',
                    backdrop: 'static'
                });
        ...
        $scope.GetstoreTagss = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "tagName", displayName: "Tag Name", width: '50%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "description", displayName: "Description", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "creationTime", displayName: "CreationTime", width: '20%', headerCellClass: $scope.highlightFilteredHeader, type: 'date', cellFilter: 'date:\'yyyy-MM-dd h:mm:ss a\'' },
                    {
                        name: 'Commands',
                        enableFiltering: false,
                        enableSorting: false,
            ...
            //var apiRoute = 'api/storeTags/GetstoreTagss/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreTagss(apiRoute);
            ...
            //    $('.grid'),
                ...
                _thisServicesstoreTagss.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

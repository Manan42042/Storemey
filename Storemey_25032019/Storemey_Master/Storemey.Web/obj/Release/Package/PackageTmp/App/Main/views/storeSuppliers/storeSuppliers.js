...
app.controller('storeSupplierssCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeSuppliers', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreSupplierss, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeSupplierss = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreSuppliers', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreSuppliers', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreSuppliersInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreSuppliersInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Supplier, name: <strong>' + entity.supplierFullName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreSupplierss.delete(DeletestoreSuppliersInputDto).then(function (result) {
        ...
        //    _thisServicesstoreSupplierss.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeSuppliers/importModal.cshtml',
                    controller: 'importstoreSuppliersCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreSupplierss = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "supplierFullName", displayName: "SupplierFullName", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "description", displayName: "Description", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "companyName", displayName: "CompanyName", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "mobile", displayName: "Mobile", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeSuppliers/GetstoreSupplierss/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreSupplierss(apiRoute);
            ...
            //    $('.grid'),
            cfpLoadingBar.start();
                _thisServicesstoreSupplierss.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

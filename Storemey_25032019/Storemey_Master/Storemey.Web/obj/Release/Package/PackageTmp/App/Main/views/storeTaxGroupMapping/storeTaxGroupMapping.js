...
app.controller('storeTaxGroupMappingsCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeTaxGroupMapping', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreTaxGroupMappings, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeTaxGroupMappings = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreTaxGroupMapping', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreTaxGroupMapping', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreTaxGroupMappingInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreTaxGroupMappingInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Tax, name: <strong>' + entity.taxGroupName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreTaxGroupMappings.delete(DeletestoreTaxGroupMappingInputDto).then(function (result) {
        ...
        //    _thisServicesstoreTaxGroupMappings.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeTaxGroupMapping/importModal.cshtml',
                    controller: 'importstoreTaxGroupMappingCTR',
                    backdrop: 'static'
                });
        ...
        $scope.pagination = {
            ...
            pageSizeChange: function () {
                ...
                this.pageNumber = 1;
                $scope.GetstoreTaxGroupMappings();
            },
            firstPage: function () {
        ...
        //ui-Grid Call
        $scope.GetstoreTaxGroupMappings = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "taxGroupName", displayName: "Tax group name", width: '83%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeTaxGroupMapping/GetstoreTaxGroupMappings/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreTaxGroupMappings(apiRoute);
            ...
            //    $('.grid'),
                ...
                _thisServicesstoreTaxGroupMappings.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

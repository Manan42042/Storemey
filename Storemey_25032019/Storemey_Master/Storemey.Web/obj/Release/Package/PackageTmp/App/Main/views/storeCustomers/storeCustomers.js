...
app.controller('storeCustomerssCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeCustomers', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreCustomerss, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeCustomerss = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreCustomers', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreCustomers', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreCustomersInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreCustomersInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Customer, name: <strong>' + entity.firstName + ' ' + entity.lastName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreCustomerss.delete(DeletestoreCustomersInputDto).then(function (result) {
        ...
        //    _thisServicesstoreCustomerss.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeCustomers/importModal.cshtml',
                    controller: 'importstoreCustomersCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreCustomerss = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "firstName", displayName: "First Name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "lastName", displayName: "Last Name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "dateOfBirth", displayName: "Date Of Birth", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "companyName", displayName: "Company Name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeCustomers/GetstoreCustomerss/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreCustomerss(apiRoute);
            ...
            //    $('.grid'),
                ...
                _thisServicesstoreCustomerss.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

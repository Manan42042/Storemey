...
angular.module('app').controller('adminCurrancyCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeCurrencies','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesadminCurrancy, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.adminCurrancy = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditadminCurrancy', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditadminCurrancy', { SearchDto: $scope.SearchDto });
        }

        var DeleteadminCurrancyInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeleteadminCurrancyInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesadminCurrancy.delete(DeleteadminCurrancyInputDto).then(function (result) {
        ...
        //    _thisServicesadminCurrancy.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/adminCurrancy/importModal.cshtml',
                    controller: 'importadminCurrancyCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetadminCurrancy = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    { name: "currancy_Name", displayName: "Name", width: '27%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "currency_Symbol", enableSorting: false, displayName: "Symbol", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "currency_Code", displayName: "Code", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "currency_Code02", displayName: "Code02", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/adminCurrancy/GetadminCurrancy/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getadminCurrancy(apiRoute);
            ...
            //    $('.grid'),
            cfpLoadingBar.start();
                _thisServicesadminCurrancy.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

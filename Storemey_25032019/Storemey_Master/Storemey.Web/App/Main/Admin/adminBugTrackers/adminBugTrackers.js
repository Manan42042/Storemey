...
angular.module('app').controller('adminBugTrackersCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.adminBugTrackers','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesadminBugTrackers, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.adminBugTrackers = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditadminBugTrackers', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditadminBugTrackers', { SearchDto: $scope.SearchDto });
        }

        var DeleteadminBugTrackersInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeleteadminBugTrackersInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesadminBugTrackers.delete(DeleteadminBugTrackersInputDto).then(function (result) {
        ...
        //    _thisServicesadminBugTrackers.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/adminBugTrackers/importModal.cshtml',
                    controller: 'importadminBugTrackersCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetadminBugTrackers = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "title", title: "Title", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "bugDate", title: "Bug Date", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "bugFromName", title: "Bug From Name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "bugStoreName", title: "Store Name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/adminBugTrackers/GetadminBugTrackers/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getadminBugTrackers(apiRoute);
            ...
            //    $('.grid'),
            cfpLoadingBar.start();
                _thisServicesadminBugTrackers.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

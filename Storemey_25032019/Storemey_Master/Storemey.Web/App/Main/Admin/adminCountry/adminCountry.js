...
angular.module('app').controller('adminCountrysCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.masterCountries', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesadminCountrys, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.adminCountrys = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditadminCountry', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditadminCountry', { SearchDto: $scope.SearchDto });
        }

        var DeleteadminCountryInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeleteadminCountryInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesadminCountrys.delete(DeleteadminCountryInputDto).then(function (result) {
        ...
        //    _thisServicesadminCountrys.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/adminCountry/importModal.cshtml',
                    controller: 'importadminCountryCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetadminCountrys = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "name", displayName: "Name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "code", displayName: "Code", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "dail_Code", displayName: "Dail_Code", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "currency_Name", displayName: "Currency_Name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "curreny_Symbol", displayName: "Curreny_Symbol", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "current_Code", displayName: "Current_Code", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    {
                        name: 'Commands',
                        enableFiltering: false,
                        enableSorting: false,
            ...
            //var apiRoute = 'api/adminCountry/GetadminCountrys/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getadminCountrys(apiRoute);
            ...
            //    $('.grid'),
            cfpLoadingBar.start();
            _thisServicesadminCountrys.getAdvanceSearch($scope.SearchDto).then(function (result) {
                if (result.data === null) {

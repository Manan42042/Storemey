...
app.controller('storecurrenciessCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeCurrencies', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstorecurrenciess, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storecurrenciess = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstorecurrencies', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstorecurrencies', { SearchDto: $scope.SearchDto });
        }

        var DeletestorecurrenciesInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestorecurrenciesInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Currancy, name: <strong>' + entity.currancy_Name + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstorecurrenciess.delete(DeletestorecurrenciesInputDto).then(function (result) {
        ...
        //    _thisServicesstorecurrenciess.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storecurrencies/importModal.cshtml',
                    controller: 'importstorecurrenciesCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.Getstorecurrenciess = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "currency", displayName: "Currency", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "symbol", enableSorting: false, displayName: "Symbol", width: '10%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "name", displayName: "Name", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "country", displayName: "Country", width: '25%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "digital_code", displayName: "Digital code", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    //{ name: "lastModificationTime", displayName: "Updated Date", type: 'date', width: '26%', cellFilter: 'date:\'MM-dd-yyyy hh:mm a\'', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storecurrencies/Getstorecurrenciess/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstorecurrenciess(apiRoute);
            ...
            cfpLoadingBar.start();
                ...
                _thisServicesstorecurrenciess.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

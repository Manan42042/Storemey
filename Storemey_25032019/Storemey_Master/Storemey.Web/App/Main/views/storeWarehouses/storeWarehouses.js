...
app.controller('storeWarehousesCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeWarehouses', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreWarehouses, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeWarehouses = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreWarehouses', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreWarehouses', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreWarehousesInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreWarehousesInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreWarehouses.delete(DeletestoreWarehousesInputDto).then(function (result) {
        ...
        //    _thisServicesstoreWarehouses.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeWarehouses/importModal.cshtml',
                    controller: 'importstoreWarehousesCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreWarehouses = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "warehouseName", displayName: "Warehouse name", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "state", displayName: "State", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "city", displayName: "City", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "contactNumber", displayName: "Contact number", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "mobileNumber", displayName: "Mobile number", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "email", displayName: "Email", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeWarehouses/GetstoreWarehouses/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreWarehouses(apiRoute);
            ...
            cfpLoadingBar.start();
                ...
                _thisServicesstoreWarehouses.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

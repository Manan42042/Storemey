...
app.controller('storeCityMastersCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeCityMaster', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreCityMasters, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeCityMasters = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreCityMaster', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreCityMaster', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreCityMasterInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreCityMasterInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'City, name: <strong>' + entity.cityName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreCityMasters.delete(DeletestoreCityMasterInputDto).then(function (result) {
        ...
        //    _thisServicesstoreCityMasters.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeCityMaster/importModal.cshtml',
                    controller: 'importstoreCityMasterCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreCityMasters = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "cityName", displayName: "City name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "stateName", displayName: "State name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "countryName", displayName: "Country name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeCityMaster/GetstoreCityMasters/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreCityMasters(apiRoute);
            ...
            //    $('.grid'),
                ...
                _thisServicesstoreCityMasters.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

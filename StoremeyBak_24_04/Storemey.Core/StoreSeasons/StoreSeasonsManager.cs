...
{
    public class StoreSeasonsManager : IStoreSeasonsManager
    {
        private readonly IRepository<StoreSeasons, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreSeasonsManager(
            IRepository<StoreSeasons, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreSeasons>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreSeasons>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                             || x.SeasonName.Contains(SearchText)
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "seasonName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.SeasonName);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "seasonName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.SeasonName);
                                break;
        ...
        public async Task Create(StoreSeasons input)
        {
            try
            {

...
namespace Storemey.StoreProducts
{
    public class StoreProductsManager : IStoreProductsManager
    {
        ...
        public async Task<Tuple<IQueryable<StoreProducts>,int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "productName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.ProductName);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "productName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.ProductName);
                                break;
        ...
        public async Task Create(StoreProducts input)
        {
            try
            {
        ...
        public async Task Update(StoreProducts input)
        {

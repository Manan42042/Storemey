...
using Storemey.StoreCashRegisterInOut;
using Storemey.Authorization.Users;
using System.Data.Entity.SqlServer;

namespace Storemey.StoreCashRegisterInOut
{
    public class StoreCashRegisterInOutManager : IStoreCashRegisterInOutManager
    {
        private readonly IRepository<StoreCashRegisterInOut, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreCashRegisterInOutManager(
            IRepository<StoreCashRegisterInOut, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreCashRegisterInOut>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreCashRegisterInOut>, int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)


        //public async Task<IQueryable<StoreCashRegisterInOut>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                             || x.Transaction.Contains(SearchText)
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "transaction":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.Transaction);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "transaction":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.Transaction);
                                break;
        ...
        public async Task Create(StoreCashRegisterInOut input)
        {
            try
            {
        ...
        public async Task Update(StoreCashRegisterInOut input)
        {
            await _thisRpository.UpdateAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task Delete(StoreCashRegisterInOut input)
        {
            await _thisRpository.DeleteAsync(input);
            _unitOfWorkManager.Current.SaveChanges();
        }

        public async Task<StoreCashRegisterInOut> GetByID(long ID)
        {
            var @MasterCountries = await _thisRpository.FirstOrDefaultAsync(x => x.IsDeleted == false && x.Id == ID);

            if (@MasterCountries == null)
            {
                return new StoreCashRegisterInOut();
            }
            return @MasterCountries;
        }

...
{
    public class StoreTaxGroupsManager : IStoreTaxGroupsManager
    {
        private readonly IRepository<StoreTaxGroups, Guid> _thisRpository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;
        private readonly UserRegistrationManager _userRegistrationManager;

        public StoreTaxGroupsManager(
            IRepository<StoreTaxGroups, Guid> myMasterRepository,
        ...
        public async Task<IEnumerable<StoreTaxGroups>> ListAll()
        {
            try
            {
        ...
        public async Task<Tuple<IQueryable<StoreTaxGroups>,int>> ListAllQueryable(string SearchText, int CurrentPage, int MaxRecords, string SortColumn, string SortDirection)
        {
            try
            {
                int recordCount = 0;
                ...
                if (!string.IsNullOrEmpty(SearchText))
                {
                    @MasterCountries = @MasterCountries.Where(x => x.IsDeleted == false && SqlFunctions.StringConvert((double)x.Id).Contains(SearchText)
                                             || x.TaxGroupName.Contains(SearchText)
                    ...
                    case "desc":
                        ...
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "taxGroupName":
                                @MasterCountries = @MasterCountries.OrderByDescending(x => x.TaxGroupName);
                                break;
                    ...
                    default:
                        switch (SortColumn)
                        {
                            case "id":
                                ...
                                break;
                            case "taxGroupName":
                                @MasterCountries = @MasterCountries.OrderBy(x => x.TaxGroupName);
                                break;
        ...
        public async Task Create(StoreTaxGroups input)
        {
            try
            {

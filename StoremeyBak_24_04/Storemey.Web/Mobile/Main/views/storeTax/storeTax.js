...
app.controller('storeTaxsCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeTax','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreTaxs, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeTaxs = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreTax', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreTax', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreTaxInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreTaxInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Tax, name: <strong>' + entity.taxName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreTaxs.delete(DeletestoreTaxInputDto).then(function (result) {
        ...
        //    _thisServicesstoreTaxs.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeTax/importModal.cshtml',
                    controller: 'importstoreTaxCTR',
                    backdrop: 'static'
                });
        ...
        $scope.GetstoreTaxs = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "taxName", displayName: "Tax name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "rate", displayName: "Rate (%)", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "isDefault", displayName: "Is default", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeTax/GetstoreTaxs/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreTaxs(apiRoute);
            ...
            //    $('.grid'),
            cfpLoadingBar.start();
                _thisServicesstoreTaxs.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

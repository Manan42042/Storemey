...
app.controller('storeOutletsCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeOutlets', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreOutlets, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeOutlets = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreOutlets', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreOutlets', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreOutletsInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreOutletsInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreOutlets.delete(DeletestoreOutletsInputDto).then(function (result) {
        ...
        //    _thisServicesstoreOutlets.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeOutlets/importModal.cshtml',
                    controller: 'importstoreOutletsCTR',
                    backdrop: 'static'
                });
        ...
        $scope.GetstoreOutlets = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "outletName", displayName: "Outlet name", width: '13%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "warehouseName", displayName: "Warehouse name", width: '13%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "state", displayName: "State", width: '13%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "city", displayName: "City", width: '13%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "timeZone", displayName: "Timezone", width: '13%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "contactNumber", displayName: "Contact number", width: '12%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "email", displayName: "Email", width: '13%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeOutlets/GetstoreOutlets/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreOutlets(apiRoute);
            ...
            cfpLoadingBar.start();
                ...
                _thisServicesstoreOutlets.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

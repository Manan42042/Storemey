...
app.controller('storeSeasonssCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storeSeasons','cfpLoadingBar', 
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstoreSeasonss, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storeSeasonss = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstoreSeasons', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstoreSeasons', { SearchDto: $scope.SearchDto });
        }

        var DeletestoreSeasonsInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestoreSeasonsInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Season, name: <strong>' + entity.SeasonName + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstoreSeasonss.delete(DeletestoreSeasonsInputDto).then(function (result) {
        ...
        //    _thisServicesstoreSeasonss.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storeSeasons/importModal.cshtml',
                    controller: 'importstoreSeasonsCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstoreSeasonss = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "seasonName", displayName: "Season Name", width: '30%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "description", displayName: "Description", width: '60%', headerCellClass: $scope.highlightFilteredHeader },
            ...
            //var apiRoute = 'api/storeSeasons/GetstoreSeasonss/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstoreSeasonss(apiRoute);
            ...
            cfpLoadingBar.start();
                ...
                _thisServicesstoreSeasonss.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

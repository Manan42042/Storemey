...
app.controller('storePaymentTypessCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.storePaymentTypes', 'cfpLoadingBar',
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesstorePaymentTypess, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.storePaymentTypess = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditstorePaymentTypes', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditstorePaymentTypes', { SearchDto: $scope.SearchDto });
        }

        var DeletestorePaymentTypesInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeletestorePaymentTypesInputDto.id = entity.id;


            $ngConfirm({
                title: 'Remove record ',
                content: 'Payment type, name: <strong>' + entity.name + '</strong> <div> Are you sure you want to delete this record? </div> ',
                scope: $scope,
                buttons: {
                    Yes: {
                        text: 'YES',
                        ...
                        action: function (scope, button) {
                            _thisServicesstorePaymentTypess.delete(DeletestorePaymentTypesInputDto).then(function (result) {
        ...
        //    _thisServicesstorePaymentTypess.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/storePaymentTypes/importModal.cshtml',
                    controller: 'importstorePaymentTypesCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetstorePaymentTypess = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "name", displayName: "Paymenttype name", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    //{ name: "isPOSPaymentType", displayName: "Is pos payment type", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    //{ name: "isEcommarcePaymentType", displayName: " Is ecommarce payment type", width: '20%', headerCellClass: $scope.highlightFilteredHeader },


                    {
                        name: 'Is pos payment type',
                        enableFiltering: false,
                        enableSorting: false,
                        width: '20%',
                        enableColumnResizing: false,
                        cellTemplate: '<span class="label-mini">' +
                            '<img src="/Images/{{row.entity.isPOSPaymentType}}.png" style="margin-top: 10px;" />' +
                            '</span>'
                    },


                    {
                        name: 'Is ecommarce payment type',
                        enableFiltering: false,
                        enableSorting: false,
                        width: '20%',
                        enableColumnResizing: false,
                        cellTemplate: '<span class="label-mini">' +
                            '<img src="/Images/{{row.entity.isEcommarcePaymentType}}.png" style="margin-top: 10px;" />' +
                            '</span>'
                    },
                    { name: "lastModificationTime", displayName: "Date updated", width: '30%', headerCellClass: $scope.highlightFilteredHeader, type: 'date', cellFilter: 'date:\'yyyy-MM-dd h:mm:ss a\'' },
                    {
                        name: 'Commands',
                        enableFiltering: false,
                        enableSorting: false,
            ...
            //var apiRoute = 'api/storePaymentTypes/GetstorePaymentTypess/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getstorePaymentTypess(apiRoute);
            ...
            //    $('.grid'),
                ...
                _thisServicesstorePaymentTypess.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {

...
angular.module('app').controller('adminSMTPsettingsCtrl', ['$scope', '$ngConfirm', '$state', '$stateParams', '$uibModal', 'abp.services.app.adminSMTPsettings','cfpLoadingBar' ,
    function ($scope, $ngConfirm, $state, $stateParams, $uibModal, _thisServicesadminSMTPsettings, cfpLoadingBar) {
        $scope.gridOptions = [];
        $scope.adminSMTPsettings = [];
        ...
        $scope.edit = function (entity) {
            ...
            //$state.go('addeditadminSMTPsettings', { 'id': entity.id, 'searchDto': $scope.SearchDto });
            $scope.SearchDto.tempID = entity.id;
            $state.go('addeditadminSMTPsettings', { SearchDto: $scope.SearchDto });
        }

        var DeleteadminSMTPsettingsInputDto = {
        }
        $scope.remove = function (entity) {
            //alert("remote " + JSON.stringify(entity));
            DeleteadminSMTPsettingsInputDto.id = entity.id;
                ...
                buttons: {
                    Yes: {
                        ...
                        action: function (scope, button) {
                            _thisServicesadminSMTPsettings.delete(DeleteadminSMTPsettingsInputDto).then(function (result) {
        ...
        //    _thisServicesadminSMTPsettings.exportToCSV().then(function (result) {
        ...
        $scope.DropValueChange = function (item) {
            ...
            if (item === 'Export CSV') {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '/App/Main/views/adminSMTPsettings/importModal.cshtml',
                    controller: 'importadminSMTPsettingsCTR',
                    backdrop: 'static'
                });
        ...
        //ui-Grid Call
        $scope.GetadminSMTPsettings = function () {
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [
                    ...
                    { name: "host", title: "Host", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "port", title: "Port", width: '15%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "userName", title: "UserName", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "password", title: "Password", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    { name: "enableSSL", title: "EnableSSL", width: '20%', headerCellClass: $scope.highlightFilteredHeader },
                    {
                        name: 'Commands',
                        enableFiltering: false,
                        enableSorting: false,
            ...
            //var apiRoute = 'api/adminSMTPsettings/GetadminSMTPsettings/' + NextPage + '/' + NextPageSize;
            //var result = CRUDService.getadminSMTPsettings(apiRoute);
            ...
            cfpLoadingBar.start();
                ...
                _thisServicesadminSMTPsettings.getAdvanceSearch($scope.SearchDto).then(function (result) {
                    if (result.data === null) {
